...
基于 [Tavily](https://tavily.com) 的 `WebSearchProvider`，注册进 harness 的 [web 能力缝](../web/README.zh.md)（`ctx.web`）。它调用 Tavily 的 `POST /search` 检索端点——专用搜索 API 而非模型调用，一次搜索只消耗检索额度——并把扁平的 `results[]` 映射为缝的规范化 `WebSearchResult`。
...
间接，经由 [`dsh-tool-web`](../tool-web/README.zh.md)：该消费方保留本 provider 按 `maxResults` 截断的 URL、标题、摘录与（开启 `includeAnswer` 时的）生成答案，或其确切的 `Tavily search aborted`、`Tavily search request failed: <error>`、`Tavily returned an unprocessable response body: <error>` 失败（包在消费方的错误包装下），provider 私有字段不进入上下文。
...
- **只暴露 `searchDepth`/`topic`/`maxResults`/`includeAnswer`** — Tavily 的按请求控制（时间范围、日期边界、域名过滤、国家加权、图片结果）等待 provider 中立的 Service Definition 字段（[缝 Agent Note](../../../.agents/notes/implemented/architecture/2026-06-24-web-capability-seam.zh.md)）；今天需要它们的部署在本缝之外使用 Tavily CLI skill 或 MCP server。
- **Tavily 的 extract 能力不在范围内** — `web_extract` 操作是缝的延期工作，不是 fetch provider 的加宽（[缝 README](../web/README.zh.md)）。
