...
describe('TavilySearchProvider redirect policy', () => {
  ...
  it('shows default 307 following forwards the POST body; the credential is stripped cross-origin', async () => {
    targetRequests.length = 0
    ...
    // Undici applies the Fetch spec's cross-origin redirect rule: the
    // Authorization header does not reach the Location. The provider's
    // `redirect: 'error'` policy above makes the question moot for its own
    // requests — nothing is contacted at all.
