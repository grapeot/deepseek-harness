...
Package tests cover `/oauth login xai` against a fake flow (verification URL and code, completion, status, pending, failure, pre-notify rejection, completion without a device code), single-flight refresh that persists before returning, rejected refresh with re-login directions, logout that wins against an in-flight refresh or login persist, `set`/`unset` on an owned reference, source disposal with the contributing fiber, and a file-layer conflict at source registration. `loadBuiltInFlow('xai')` loads the real pi-ai object. Manual SuperGrok login and one live Grok completion remain a PR-recorded check, not a keyless gate.
