...
Tavily 的按请求控制（时间范围、日期边界、域名过滤、国家加权、图片）不进来：缝的请求词表是 `query` + `maxResults`，直到 provider 中立字段存在（[缝的延期工作](../architecture/2026-06-24-web-capability-seam.zh.md)）。今天需要这些控制的部署继续在缝外使用 Tavily CLI skill 或 MCP server。
