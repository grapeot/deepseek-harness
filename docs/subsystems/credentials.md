...
A source answers `resolve` and `describe` for the references it owns and registers through `ctx.credentialSources`. The local provider folds registered sources between the inherited environment and the managed file. Two sources claiming one reference reject at registration.
